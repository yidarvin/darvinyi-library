...
- **Tier:** 2
- **Slug / route:** `why-nations-fail`  ->  `/why-nations-fail`

## Thesis to convey

Prosperity depends on inclusive institutions that share power and opportunity, while extractive ones keep nations poor.

## Signature model

**Inclusive vs. extractive institutions.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `why-nations-fail` to done in the registry and DONE in the queue.
