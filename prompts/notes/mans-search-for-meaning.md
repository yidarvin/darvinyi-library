...
- **Tier:** 1
- **Slug / route:** `mans-search-for-meaning`  ->  `/mans-search-for-meaning`

## Thesis to convey

Even in the worst suffering we retain the freedom to choose our response, and finding meaning is what lets us endure.

## Signature model

**Logotherapy.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `mans-search-for-meaning` to done in the registry and DONE in the queue.
