...
- **Tier:** 2
- **Slug / route:** `fooled-by-randomness`  ->  `/fooled-by-randomness`

## Thesis to convey

We routinely mistake luck for skill because we see the winners and never count the invisible losers.

## Signature model

**Survivorship bias.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `fooled-by-randomness` to done in the registry and DONE in the queue.
