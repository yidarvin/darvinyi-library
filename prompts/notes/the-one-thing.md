...
- **Tier:** 2
- **Slug / route:** `the-one-thing`  ->  `/the-one-thing`

## Thesis to convey

Extraordinary results come from narrowing focus to the single most important task and letting it drive everything else.

## Signature model

**The focusing question.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `the-one-thing` to done in the registry and DONE in the queue.
