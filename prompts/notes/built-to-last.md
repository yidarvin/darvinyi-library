...
- **Tier:** 2
- **Slug / route:** `built-to-last`  ->  `/built-to-last`

## Thesis to convey

Enduring great companies are built on a core ideology and audacious goals, not a single great leader or product.

## Signature model

**BHAG.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `built-to-last` to done in the registry and DONE in the queue.
