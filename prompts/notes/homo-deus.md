...
- **Tier:** 2
- **Slug / route:** `homo-deus`  ->  `/homo-deus`

## Thesis to convey

Having largely tamed famine, plague, and war, humanity's next project is upgrading itself toward god-like powers, with data as the new authority.

## Signature model

**Dataism.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `homo-deus` to done in the registry and DONE in the queue.
