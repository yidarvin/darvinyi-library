...
- **Tier:** 2
- **Slug / route:** `artists-way`  ->  `/artists-way`

## Thesis to convey

Creativity can be recovered through steady practices that quiet the inner critic and refill the well.

## Signature model

**Morning pages and artist dates.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `artists-way` to done in the registry and DONE in the queue.
