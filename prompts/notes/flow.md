...
- **Tier:** 2
- **Slug / route:** `flow`  ->  `/flow`

## Thesis to convey

The best moments come in a state of total absorption where challenge and skill are matched, and that state can be cultivated.

## Signature model

**The flow channel.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `flow` to done in the registry and DONE in the queue.
