...
- **Shelf:** Negotiation
- **Tier:** 1
- **Slug / route:** `getting-to-yes`  ->  `/getting-to-yes`

## Thesis to convey

Separate people from the problem and bargain over interests, not positions, to reach deals that actually hold.

## Signature model

**Principled negotiation / BATNA.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `getting-to-yes` to done in the registry and DONE in the queue.
