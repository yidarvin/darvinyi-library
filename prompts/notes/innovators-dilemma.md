...
- **Tier:** 1
- **Slug / route:** `innovators-dilemma`  ->  `/innovators-dilemma`

## Thesis to convey

Great companies fail precisely by doing everything right for current customers while disruptive newcomers redefine the market beneath them.

## Signature model

**Disruptive innovation.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `innovators-dilemma` to done in the registry and DONE in the queue.
