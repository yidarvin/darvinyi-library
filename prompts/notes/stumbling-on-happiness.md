...
- **Tier:** 3
- **Slug / route:** `stumbling-on-happiness`  ->  `/stumbling-on-happiness`

## Thesis to convey

We are strikingly bad at predicting what will make us happy, because imagination distorts the future in consistent ways.

## Signature model

**Affective forecasting.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `stumbling-on-happiness` to done in the registry and DONE in the queue.
