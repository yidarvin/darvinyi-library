...
- **Tier:** 1
- **Slug / route:** `deep-work`  ->  `/deep-work`

## Thesis to convey

The ability to focus without distraction on hard problems is rare, valuable, and trainable, and it is being destroyed by default modern work.

## Signature model

**Deep vs. shallow work.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `deep-work` to done in the registry and DONE in the queue.
