...
- **Tier:** 2
- **Slug / route:** `tao-te-ching`  ->  `/tao-te-ching`

## Thesis to convey

Live in accord with the Tao by yielding, simplifying, and acting without forcing.

## Signature model

**Wu wei.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `tao-te-ching` to done in the registry and DONE in the queue.
