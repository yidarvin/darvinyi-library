...
- **Tier:** 1
- **Slug / route:** `start-with-why`  ->  `/start-with-why`

## Thesis to convey

People do not buy what you do, they buy why you do it, so great leaders and brands start from purpose.

## Signature model

**The Golden Circle.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `start-with-why` to done in the registry and DONE in the queue.
