...
- **Tier:** 2
- **Slug / route:** `random-walk`  ->  `/random-walk`

## Thesis to convey

Markets are efficient enough that most active picking fails, so low-cost index funds are the rational default.

## Signature model

**Efficient markets.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `random-walk` to done in the registry and DONE in the queue.
