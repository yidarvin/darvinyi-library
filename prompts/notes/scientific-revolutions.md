...
- **Tier:** 3
- **Slug / route:** `scientific-revolutions`  ->  `/scientific-revolutions`

## Thesis to convey

Science does not advance smoothly but lurches through revolutions that overturn whole paradigms.

## Signature model

**Paradigm shift.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `scientific-revolutions` to done in the registry and DONE in the queue.
