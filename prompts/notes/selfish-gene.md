...
- **Tier:** 1
- **Slug / route:** `selfish-gene`  ->  `/selfish-gene`

## Thesis to convey

Evolution is best understood from the gene's point of view: bodies are vehicles genes build to copy themselves.

## Signature model

**The gene's-eye view / the meme.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `selfish-gene` to done in the registry and DONE in the queue.
