...
- **Tier:** 1
- **Slug / route:** `nudge`  ->  `/nudge`

## Thesis to convey

How choices are arranged quietly steers the decisions people make, so design the architecture deliberately.

## Signature model

**Choice architecture / the nudge.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `nudge` to done in the registry and DONE in the queue.
