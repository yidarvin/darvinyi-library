...
- **Tier:** 3
- **Slug / route:** `behave`  ->  `/behave`

## Thesis to convey

Every human behavior has causes stacked across timescales, from a neuron firing a second before to evolution over millennia.

## Signature model

**Nested timescales of causation.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `behave` to done in the registry and DONE in the queue.
