...
- **Tier:** 2
- **Slug / route:** `extreme-ownership`  ->  `/extreme-ownership`

## Thesis to convey

Leaders own everything in their world, with no one else to blame, and that total responsibility is what makes teams win.

## Signature model

**Extreme ownership.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `extreme-ownership` to done in the registry and DONE in the queue.
