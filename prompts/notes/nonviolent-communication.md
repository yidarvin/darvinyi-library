...
- **Tier:** 1
- **Slug / route:** `nonviolent-communication`  ->  `/nonviolent-communication`

## Thesis to convey

Conflict softens when we speak from observations, feelings, and needs, and make clear requests instead of demands.

## Signature model

**Observation, feeling, need, request.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `nonviolent-communication` to done in the registry and DONE in the queue.
