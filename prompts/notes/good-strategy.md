...
- **Tier:** 2
- **Slug / route:** `good-strategy`  ->  `/good-strategy`

## Thesis to convey

Real strategy is a clear diagnosis, a guiding policy, and coherent action, not a list of goals and buzzwords.

## Signature model

**Diagnosis, guiding policy, coherent action.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `good-strategy` to done in the registry and DONE in the queue.
