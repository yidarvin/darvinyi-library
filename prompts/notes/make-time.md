...
- **Tier:** 3
- **Slug / route:** `make-time`  ->  `/make-time`

## Thesis to convey

Reclaim attention from an always-on world with small daily tactics built around a single chosen highlight.

## Signature model

**Highlight, Laser, Energy, Reflect.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `make-time` to done in the registry and DONE in the queue.
