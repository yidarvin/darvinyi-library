...
- **Tier:** 2
- **Slug / route:** `antifragile`  ->  `/antifragile`

## Thesis to convey

Some things gain from disorder; the goal is not just to survive shocks but to be built so they make you stronger.

## Signature model

**Fragile, robust, antifragile.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `antifragile` to done in the registry and DONE in the queue.
