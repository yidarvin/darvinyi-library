...
- **Tier:** 2
- **Slug / route:** `hard-thing`  ->  `/hard-thing`

## Thesis to convey

There is no formula for the hardest calls in building a company; there is only honesty, nerve, and doing the brutal thing well.
...
passes; then flip `hard-thing` to done in the registry and DONE in the queue.
