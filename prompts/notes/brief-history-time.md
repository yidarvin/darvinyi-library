...
- **Tier:** 2
- **Slug / route:** `brief-history-time`  ->  `/brief-history-time`

## Thesis to convey

Space, time, black holes, and the origin of the universe, made graspable without equations.
...
passes; then flip `brief-history-time` to done in the registry and DONE in the queue.
