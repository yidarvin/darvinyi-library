...
- **Tier:** 1
- **Slug / route:** `black-swan`  ->  `/black-swan`

## Thesis to convey

History is dominated by rare, high-impact events that we cannot predict but insist on explaining after the fact.

## Signature model

**Black swan events.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `black-swan` to done in the registry and DONE in the queue.
