...
- **Tier:** 1
- **Slug / route:** `influence`  ->  `/influence`

## Thesis to convey

Compliance runs on a handful of deep psychological triggers that can be understood, spotted, and defended against.

## Signature model

**Six principles of persuasion.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `influence` to done in the registry and DONE in the queue.
