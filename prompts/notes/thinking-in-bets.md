...
- **Tier:** 2
- **Slug / route:** `thinking-in-bets`  ->  `/thinking-in-bets`

## Thesis to convey

Good decisions and good outcomes are not the same thing; judge the decision, not the result.

## Signature model

**Resulting.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `thinking-in-bets` to done in the registry and DONE in the queue.
