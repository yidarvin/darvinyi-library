...
- **Tier:** 2
- **Slug / route:** `happiness-hypothesis`  ->  `/happiness-hypothesis`

## Thesis to convey

Ancient wisdom about the good life holds up surprisingly well against modern psychology, once you see the mind as a rider on an elephant.

## Signature model

**The elephant and the rider.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `happiness-hypothesis` to done in the registry and DONE in the queue.
