...
- **Tier:** 2
- **Slug / route:** `four-hour-workweek`  ->  `/four-hour-workweek`

## Thesis to convey

Escape the deferred-life plan by automating income and outsourcing work to buy back time now.

## Signature model

**Define, Eliminate, Automate, Liberate.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `four-hour-workweek` to done in the registry and DONE in the queue.
