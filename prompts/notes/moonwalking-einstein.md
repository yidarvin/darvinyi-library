...
- **Tier:** 3
- **Slug / route:** `moonwalking-einstein`  ->  `/moonwalking-einstein`

## Thesis to convey

Extraordinary memory is a trainable technique, not a gift, and the ancient method of loci is its heart.

## Signature model

**The memory palace.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `moonwalking-einstein` to done in the registry and DONE in the queue.
