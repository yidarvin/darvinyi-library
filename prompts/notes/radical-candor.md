...
- **Tier:** 3
- **Slug / route:** `radical-candor`  ->  `/radical-candor`

## Thesis to convey

The best feedback comes from caring personally and challenging directly at the same time.

## Signature model

**The candor matrix.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `radical-candor` to done in the registry and DONE in the queue.
