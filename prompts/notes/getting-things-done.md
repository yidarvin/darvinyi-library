...
- **Tier:** 2
- **Slug / route:** `getting-things-done`  ->  `/getting-things-done`

## Thesis to convey

Your mind is for having ideas, not holding them; get everything into a trusted external system and act from there.

## Signature model

**Capture, clarify, organize, reflect, engage.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `getting-things-done` to done in the registry and DONE in the queue.
