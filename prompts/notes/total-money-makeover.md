...
- **Tier:** 3
- **Slug / route:** `total-money-makeover`  ->  `/total-money-makeover`

## Thesis to convey

Get out of debt with a strict, emotional, step-by-step plan built on small early wins.

## Signature model

**The debt snowball.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `total-money-makeover` to done in the registry and DONE in the queue.
