...
- **Tier:** 1
- **Slug / route:** `atomic-habits`  ->  `/atomic-habits`

## Thesis to convey

Tiny changes compound: build systems and identity rather than chasing goals, and let small habits accumulate into remarkable results.

## Signature model

**Four laws of behavior change.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `atomic-habits` to done in the registry and DONE in the queue.
