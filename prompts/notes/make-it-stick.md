...
- **Tier:** 2
- **Slug / route:** `make-it-stick`  ->  `/make-it-stick`

## Thesis to convey

The learning that feels easy is the least durable; real learning comes from retrieval, spacing, and desirable difficulty.

## Signature model

**Retrieval, spacing, interleaving.** Render this as the hero diagram in the Model section, in house style, as an original recreation of the idea (never a trace of the book's own figure).
...
passes; then flip `make-it-stick` to done in the registry and DONE in the queue.
